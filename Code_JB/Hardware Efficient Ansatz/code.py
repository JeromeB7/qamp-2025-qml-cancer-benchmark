...
def hardware_efficient_ansatz(n_features, reps=2):
    qc = QuantumCircuit(n_features)

    # Parameters: Data encoding (x) + trainable params (theta: RY + RZ per layer per qubit)
    x_params = ParameterVector("x", n_features)
    theta_params = ParameterVector("θ", reps * n_features * 2)  # RY + RZ per qubit per rep

    # Initial data encoding layer (hardware-efficient: RX for data)
    ...
    for r in range(reps):
        ...
        for i in range(n_features):
            ...
            idx += 1
        # Entangling layer: Linear chain of CX (hardware-efficient, respects nearest-neighbor connectivity)
...
def build_trainable_kernel(n_features, reps=2):
    feature_map, training_parameters = hardware_efficient_ansatz(n_features, reps)
